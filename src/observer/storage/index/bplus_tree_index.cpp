    ...
    const std::vector<const FieldMeta *> &field_metas, const bool unique)
{
  if (inited_) {
    LOG_WARN("Failed to create index due to the index has been created before. file_name:%s, index:%s, field:%s",
        file_name, index_meta.name(), index_meta.fields().data());
  ...
  RC                 rc  = index_handler_.create(unique, table->db()->log_handler(), bpm, file_name, field_metas);
  if (RC::SUCCESS != rc) {
    ...
    Table *table, const char *file_name, const IndexMeta &index_meta, const std::vector<const FieldMeta *> &field_metas)
{
  ...
  if (RC::SUCCESS != rc) {
    LOG_WARN("Failed to open index_handler, file_name:%s, index:%s, field:%s, rc:%s",
        file_name, index_meta.name(), index_meta.fields().data(), strrc(rc));
    return rc;
  }
...
{
  if (inited_) {
    LOG_INFO("Begin to close index, index:%s, field:%s", index_meta_.name(), index_meta_.fields().data());
