...
RC LogicalPlanGenerator::create_plan(DeleteStmt *delete_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  // unique_ptr<LogicalOperator> table_get_oper;
  // table_get_oper.reset(new TableGetLogicalOperator(table, fields, ReadWriteMode::READ_WRITE));
  ...
  //       ->set_predicates(std::move(predicate_oper->expressions()));
  //   // predicate_oper->add_child(std::move(table_get_oper));
  //   // LOG_INFO("predicate operator expression type:%d", predicate_oper->expressions().at(0)->type());
  // }

  // delete_oper->add_child(std::move(table_get_oper));
  // LOG_INFO("delete operator children size:%d", delete_oper->children().size());
